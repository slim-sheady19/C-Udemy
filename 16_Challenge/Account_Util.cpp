...
{
    std::cout << "\n=== Accounts===========================================" << std::endl;
    for (const auto acc: accounts) //remove ref since we have pointer
        std::cout << *acc << std::endl;	//dereference pointer acc
}

// Deposits supplied amount to each Account object in the vector
void deposit(std::vector<Account *> &accounts, double amount) {
    std::cout << "\n=== Depositing to Accounts =================================" << std::endl;
    for (auto acc:accounts) //remove ref because we have pointer
{
        if (acc->deposit(amount)) //change operator to -> since we have pointer
            std::cout << "Deposited " << amount << " to " << *acc << std::endl; //dereference pointer acc to access object
        else
            std::cout << "Failed Deposit of " << amount << " to " << *acc << std::endl; //dereference pointer acc to access object
    }
}
...
void withdraw(std::vector<Account *> &accounts, double amount)
{
    ...
    for (auto acc:accounts) //remove ref
	{
        if (acc->withdraw(amount)) //change operator to -> since we have pointer
            std::cout << "Withdrew " << amount << " from " << *acc << std::endl;	//dereference pointer acc to access object
        else
            std::cout << "Failed Withdrawal of " << amount << " from " << *acc << std::endl;	//dereference pointer acc to access object
    } 
}

 //REMOVING BECAUSE THIS IS ALL STATIC BINDING and we are interested in dynamic binding
